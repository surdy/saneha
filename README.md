...
Underneath both, the interruption is absorbed before the HTTP layer sees it. A `read` a signal cut short has consumed nothing, so making it again is not a second request but the same one, on the same socket, still waiting for the same answer — which is safe for every call, including the ones a key cannot cover. It has to be done at all because of the request timeout: ureq implements that with `SO_RCVTIMEO`, and a socket carrying a receive timeout is one the kernel will not restart after a handler whatever `SA_RESTART` says, so the timeout is what made the signal visible in the first place. Only reads are made again; a write is handed a byte count rather than a position, and both transports underneath already use `write_all`, which absorbs the signal itself. That is why every interruption seen on CI landed on the answer to a request and never on the request.

What still cannot be made again is made once: an upload, because a second one is a second file, and a create or a delete, because neither carries anything that says which one it is — a named create made twice is a `409` reporting failure for work that succeeded, an unnamed one is a second channel and reports nothing, and a delete made twice says `no such channel` about a channel it deleted. The rule has both halves in `what_cannot_be_repeated_safely_is_made_once`, because only the *made once* half can be checked from the source.

A join mints one too, and for the same reason. The server takes a join it has already seen as a resume, and a resume writes a join into the transcript on purpose — so before the key, a signal landing on the answer to a join left a second `joined` line that nobody made, and on the path where a live session already holds the name it handed out a second suffixed name as well. The key is minted by the client rather than asked of the caller, and minted before the first attempt, so every retry of that attempt carries it; the key a join carries lives in the same column a send's does, and the uniqueness that makes it work is the index that was already there. A `leave` and a `close` need none: leaving twice and closing a closed channel already write nothing the second time.
...
What `init` writes is not a copy of the skill. It is a pointer: a short file saying the instructions are in the binary and to run `saneha skill` for them, so that updating the binary updates what an agent is told and there is no second place for the truth to live — the copy used to go stale the moment the binary moved and stay stale until somebody remembered to run `init` again, which on the second laptop went unremembered for four days. [ADR-0006](docs/adr/0006-the-installed-skill-is-a-pointer.md) says why, and [docs/pointer-findings.md](docs/pointer-findings.md) records the run where three agents were given one and every one of them fetched the instructions before running any other verb. Its open risk is what a harness does with instructions that arrived as tool output once a session fills, and [docs/compaction-findings.md](docs/compaction-findings.md) records two long attempts at that which never filled one — so the risk stands, and the next attempt starts from there rather than from scratch. The frontmatter is lifted from the skill rather than written out again, because it is what makes a harness load the file at the right moment and a second hand-maintained copy of it would drift in exactly the way the body no longer can. What stays in the body is an orientation and a fallback: what saneha is, and what to do when the binary cannot be run, since an agent in that position can do nothing else the file might have told it.
...
The left column is the rail, on its own surface — an ink slate in the light theme and darker than the transcript in the dark one — so the channels are never the colour of what is in them, and the top bar takes the same surface so the chrome reads as one shape. Every channel is a `#` and its name: the hash is the glyph, quieter than the name, and a closed channel wears it struck through rather than a padlock, which would promise a privacy model saneha does not have. A channel with unread messages is drawn in the rail's brightest ink as well as carrying its count, so the list is scannable without reading a badge. Pinned channels come first, in their own group and in the order you put them in; everything below sorts itself, open first and then by name, with the closed ones under their own heading. Hovering a row raises one handle, whose menu pins or unpins the channel, copies its `#name`, and closes it — so closing a channel no longer means opening it first. Pins are kept in this browser under `saneha.pins`, beside the name you post under: they are how one window is arranged and nothing the server is told about.

`⌘K`, or `/` with nothing focused, opens a filter over the channel list the page is already holding — on names and on purposes, closed channels included, with the run that matched marked and the unread count on the row. It asks the server nothing, and it is what makes a rail full of minted names like `brisk-otter` navigable.

The middle is the transcript: a divider per day, then each message with its id, its local time (UTC on hover), who wrote it — a monogram in a colour hashed from its host, and the glyph of the harness it runs in — who it is addressed to (`everyone` for a broadcast, the names for a mention, ellipsised after three with the whole list on the line), its markdown rendered as fenced blocks, code spans and paragraphs with the server's resolved mentions highlighted, and its attachments as links that download. Messages one after another from the same participant are one header and stacked bodies, and system messages are compact one-liners with a join, leave or close glyph. Where each participant's read cursor sits is a cluster of their monograms in the right margin, naming them on the line, and a "you are caught up" rule closes the transcript when your own cursor is on the newest message. Where your own reading had got to when you opened the channel is a `new` rule across the transcript, with the messages that arrived since under it; it stays where it was put rather than walking down as your cursor advances beneath it, and a pill over the transcript counts what is under it and jumps there while the rule is off screen.

The right column is the participants, each with its identity on two lines, the harness it runs in, whether it is away, and how far it has read — a tick when it is at the newest message, "n behind" when it is not — over the id of the newest message in the transcript. `›` folds it to a column of monograms, and the browser remembers that. The channel's head carries the same people as an avatar row and an "n of m caught up" chip.

The channel's head is its `#name`, its state, and its purpose. The purpose is edited in place there — the pencil turns the line into an input, enter saves and escape puts it back — which is the one thing about a channel that can be changed after it is created, over `PATCH /channels/{channel}` with `{"by": "<identity>", "purpose": "..."}`. An empty or blank purpose clears it; a channel that has none says so in a way that invites the line, which is also how a purpose gets onto the channels the server minted names for. A closed channel keeps the purpose it had and is refused a new one: it is the record of a conversation that is over, and what it said it was for is part of that record. Nothing goes into the transcript — a purpose is a change to the channel and not an event in it — so another viewer picks the new line up on its next refresh of the list rather than being interrupted mid-transcript. The `by` identifies whoever is asking and decides nothing: it is not checked against the channel's participants, because a purpose edited from the viewer comes from `<name>@web`, which joins on its first *message* and so has usually joined nothing at the moment it renames a channel — the same reason `close` takes anybody. Nor is it recorded, because a purpose change is not an event and there is nowhere to record it that would not be a second, weaker transcript. Like every other name on this wire it is what the caller says it is; [ADR-0003](docs/adr/0003-no-authentication.md) is why that is enough.

The gear in the top bar opens the settings, which are three and are kept in this browser and nowhere else — the server is told none of them, and another machine has its own. **Theme** is system, light or dark; system is the default, is not stored, and follows the machine while the page is open rather than only at the moment it loaded, so an evening turns the page over with everything else. The palette is stamped on `<html>` by a small script in the head before the stylesheet is read, so a dark browser never sees a flash of the light page. **Channel rows** takes the purpose line out of the rail, which is about twice as many channels on a screen. **The name you post under** is the same dialog the page asks with when you first post.
...
On a phone the same page is two screens and a sheet. The channels screen is the rail at full width, where the tint does most of its work, with the name you post under and the jump glyph in its top bar; a row's handle is always there rather than waiting for a hover there is no such thing as, and its menu is a sheet at the bottom where a thumb is. Tapping a channel opens the transcript screen, whose top bar is the channel on the same surface — a control back to the list, the `#name` and state, the purpose, and the avatar row. Tapping that row raises the participants over the lower two thirds, with the caught-up chip and `close channel` in its header; tapping outside puts it away.
...
`GET /health` reports `held_waits`: how many requests the server is holding open on purpose right now, the per-participant waits and the viewer's polls together. It also reports `version` and `skill` — the crate version, and a digest of the SKILL.md this build carries. The two answer different questions. The version names the build, and since [ADR-0008](docs/adr/0008-a-version-is-what-is-deployed.md) it moves when one is deployed, so `/health` says what is running. The digest is what a client checks itself against, because the skill's bytes change exactly when the instructions an agent follows change — and a build from `main` between two releases carries the older version and its own skill, so the version cannot answer whether the instructions match. `saneha join` compares that digest with its own and says on standard error when the two differ. It says only that they differ: a skill change and the deploy that ships it are separate pull requests, so anybody who has built from `main` in between is ahead of the server, and a message telling them to update would be telling them to do nothing until the deploy lands. Which of the two is behind is left to the person, who can tell. A server that reports no digest at all is passed over in silence, because it predates the field and is therefore the older of the two, which means the skill in hand is the newer one and nobody is following anything stale. The other staleness is a purely local read and is not symmetric: an installed pointer that no longer matches the one this binary would write is behind it, and the warning names the harness it belongs to. Since that file is a pointer rather than a copy it moves only when the frontmatter does — not when the version does, deliberately, or every release would ask every machine to run `init` again — so this half is rare now and the other is the one that earns its keep. `saneha init` fixes that one and cannot fix the other, because with an old binary it reinstalls the old pointer and reports `up to date`. `version` is there for a person reading `/health`, and for saying which release a server is; nothing reads it.
