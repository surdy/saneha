...
On a phone the same page is two screens and a sheet. The channels screen is the list at full width, with the name you post under in its top bar; tapping a channel opens the transcript screen, whose top bar is the channel — a control back to the list, the name and state, the purpose, and the avatar row. Tapping that row raises the participants over the lower two thirds, with the caught-up chip and `close channel` in its header; tapping outside puts it away.
