...
`GET /health` reports `held_waits`: how many requests the server is holding open on purpose right now, the per-participant waits and the viewer's polls together. It also reports `version` and `skill` — the crate version, and a digest of the SKILL.md this build carries. The digest is the useful half: the crate version has not moved since the first commit, and a version nobody bumps is a check that never fires, whereas the skill's bytes change exactly when the instructions an agent follows change. `saneha join` compares that digest with its own and says on standard error when they differ, so an agent finds out its instructions are behind the deployed saneha rather than working from them for a week. It says the other staleness too, which is a purely local read: an installed `SKILL.md` that no longer matches this binary's, naming the harness it belongs to. The two want different answers — `saneha init` fixes the second and cannot fix the first, because with an old binary it reinstalls the old skill and reports `up to date`.
