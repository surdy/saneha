...
- **Nothing enforces that the tag and the deployed image agree.** ~~They agree because the same pull request carries both.~~ They disagreed on the first deploy that tested it, and carrying both in one pull request is *why*: the image a deploy pins is built from a commit that predates the deploy's own version bump, so the binary that ships reports the release before the one being cut. 0.3.0 went out as `sha-b11543e`, built from the commit before the merge that raised `Cargo.toml`, and `/health` said `0.2.0` under a `v0.3.0` tag. A deploy pull request can never pin an image containing its own bump, because that image does not exist until it merges.

  **So the version bump belongs to the change, not to the deploy.** A pull request that alters what the server does raises `Cargo.toml` to the version its change will go out under; the deploy pull request pins the image built from that merge, writes the notes, and tags. The image it pins then carries the version being released, and `/health` answers what this server is, which is the whole point of the ADR. The cost is that `main` holds a version before it is deployed — a softer form of the decoupling refused above, and the softest one available, since the alternative is a second bookkeeping commit per deploy.

  The check named here is now worth building, because the two have disagreed once. It cannot run in CI — quadhost is on the LAN and the tailnet, and GitHub Actions has no route to it — so it is a line in the runbook's verify step rather than a workflow.
