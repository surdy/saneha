...
- The local half of the staleness check in [#63](https://github.com/surdy/saneha/pull/63) now fires almost never, because the installed file changes almost never. What still moves it is a change to the pointer's own body or to the crate version, as well as to the frontmatter — so somebody who edits the body and then sees every `join` nag has an answer here. It is still correct, and the half that matters — this binary against the server's — is untouched. Between them, the copy that used to go stale silently cannot, and the binary that still can says so. Its message named the skill where what is behind is the pointer; the follow-up change made it say "the saneha pointer installed for X is behind this binary".
- The skill told an agent that a warning naming its harness means "run `saneha init` and read it again" (`skills/saneha/SKILL.md`). After this change the re-read would return the same `saneha skill` output the agent already holds, so the second half of that sentence cost a call and taught nothing. Deferred here rather than reword the same paragraph a third time in three changes, and fixed in the follow-up: the skill now says only to run `saneha init`.
